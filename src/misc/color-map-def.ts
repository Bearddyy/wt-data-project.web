...
const getBrHeatmapColorMaps: (isColorblind: boolean, isGrey: boolean) => BrHeatColorMap = (isColorblind, isGrey) => {
...
Container.bind(BrHeatColorMap).toDynamicValue(() => {
    ...
    const isGreyMode = d3.select("#color-grey-checkbox").property("checked") as boolean;
    return getBrHeatmapColorMaps(isColorblindMode, isGreyMode)
});
